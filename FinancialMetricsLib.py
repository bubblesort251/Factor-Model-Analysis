...
def calc_annualized_mean_return_metrics_geo(data, listOfFactors, freq='monthly'):
    if(freq=='monthly'):
        #print(np.mean(np.log(data[listOfFactors]+1)))
        out = pd.DataFrame((np.mean(np.log(data[listOfFactors]+1)) +1)**12-1, columns=['Annual Mean Return'])
        out['Annual SD'] = (12**.5)*data[listOfFactors].std()
        return out
    elif(freq=='daily'):
        out = pd.DataFrame((np.mean(np.log(data[listOfFactors]+1)) +1)**252-1, columns=['Annual Mean Return'])
        out['Annual SD'] = (252**.5)*data[listOfFactors].std()
        return out
    else:
        print('Incorrect Freq Specification')
