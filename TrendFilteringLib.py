...
def plot_returns_regime_multiple(data, factorNames, regimeCol, flag='Total Return', date='Date', ymaxvar=None, pathToSavePlot = False):
    '''plot_returns returns a plot of the returns
    INPUTS:
        factorName: list, elements are strings which name the column to be plotted
        data: pd dataframe, where the data is housed
        regimeCol: string, name of the regime column in the pandas df: data
        flag: string, Either Total Return or Monthly Return
        date: string, column name corresponding to the date variable
    ...
    for factor in factorNames:
        if(factor not in data.columns):
            print ('column ' + factor + ' not in pandas df')
            return
    #If the inputs are clean, create the plot
    ...
    if (flag == 'Total Return'):
        data2 = data.copy()
        data2[factorNames] = data2[factorNames] + 1
        data2[factorNames] = data2[factorNames].cumprod()
        if(ymaxvar is None):
            ymaxvar = data2[factorNames].max().max()
            yminvar = data2[factorNames].min().min()
        ...
        plt.xlabel('Date')
        plt.xlim(data2[date].iloc[0],data2[date].iloc[-1])
        plt.legend(factorNames)
        plt.ylim((.1,ymaxvar))
        if(pathToSavePlot):
            path = pathToSavePlot + 'RegimeGraph.png'
            plt.savefig(path)
